...
Vue.component('navigation', {
    ...
    <div class="row navigation  text-center">
        <div class="col paginas">
            ...
            <div v-for="(i, index) in scenes+1" v-if="onlypages==undefined && (index*10)<totalscenes.length" :class="
                [
                'pagina ',
            ...
            <div :class="[
                'pagina ',
                (totalscenes.length == currentScene ? 'iscurrent':''),

            ]" @click="$emit('goto', totalscenes.length)"><span>F</span></div>
        
        
        </div>
        <div class="col-4 col-md-3" v-if="onlypages==undefined">
            <button @click="$emit('back')" :disabled="currentScene == 0">Anterior</button>
            <button @click="$emit('next')" :disabled="currentScene == (totalscenes.length)">Siguiente</button>
        </div>
    </div>
    `
})
